...
def test_namespace_isolation_in_shared_file(db_path):
    c1 = LyteCache(db_path, namespace="ns1", sweep_interval=None)
    c2 = LyteCache(db_path, namespace="ns2", sweep_interval=None)
    try:
        c1.set("k", "from-ns1")
        c2.set("k", "from-ns2")
...
def test_context_manager(db_path):
    with LyteCache(db_path, sweep_interval=None) as cache:
        cache.set("k", "v")
        assert cache.get("k") == "v"
