...
from lytecache import CacheFullError, LyteCache
...
def test_lru_eviction_respects_order(db_path):
    cache = LyteCache(db_path, max_keys=3, eviction="lru", sweep_interval=None)
    try:
        cache.set("a", 1)
        time.sleep(_STEP)
        cache.set("b", 2)
...
def test_max_keys_enforced(db_path):
    cache = LyteCache(db_path, max_keys=5, eviction="lru", sweep_interval=None)
    try:
        for i in range(20):
            cache.set(f"k{i}", i)
...
def test_noeviction_raises_when_full(db_path):
    cache = LyteCache(db_path, max_keys=2, eviction="noeviction", sweep_interval=None)
    try:
        cache.set("a", 1)
        cache.set("b", 2)
...
def test_noeviction_add_raises_when_full(db_path):
    cache = LyteCache(db_path, max_keys=1, eviction="noeviction", sweep_interval=None)
    try:
        cache.set("a", 1)
        with pytest.raises(CacheFullError):
            cache.add("b", 2)
...
def test_ttl_eviction_evicts_soonest_expiry_first(db_path):
    cache = LyteCache(db_path, max_keys=2, eviction="ttl", sweep_interval=None)
    try:
...
def test_max_bytes_enforced(db_path):
    cache = LyteCache(db_path, max_bytes=400, eviction="lru", sweep_interval=None)
    try:
        for i in range(50):
...
def test_invalid_eviction_policy_raises(db_path):
    with pytest.raises(ValueError):
        LyteCache(db_path, eviction="bogus")
