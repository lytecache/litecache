...
from lytecache import LyteCache, _paths


@pytest.fixture(autouse=True)
def _clear_lytecache_path_env(monkeypatch):
    monkeypatch.delenv("LYTECACHE_PATH", raising=False)
...
def test_default_path_creates_file_and_parent_dirs(tmp_path, monkeypatch):
    ...
    try:
        ...
        assert cache.path.parent.name == "lytecache"
...
def test_default_path_classmethod_matches_instance_path(tmp_path, monkeypatch):
    fake_cache_dir = tmp_path / "platform-cache"
    monkeypatch.setattr(_paths, "platform_cache_dir", lambda: fake_cache_dir)
    resolved = LyteCache.default_path()
    cache = LyteCache()
    try:
...
def test_two_different_cwds_resolve_to_different_paths(tmp_path, monkeypatch):
    ...
    path_a = LyteCache.default_path()
    monkeypatch.chdir(project_b)
    path_b = LyteCache.default_path()

    assert path_a != path_b


def test_lytecache_path_env_override(tmp_path, monkeypatch):
    override = tmp_path / "custom" / "location.db"
    monkeypatch.setenv("LYTECACHE_PATH", str(override))
    cache = LyteCache()
    try:
...
def test_explicit_path_still_works(tmp_path):
    ...
    cache = LyteCache(explicit, sweep_interval=None)
    try:
        cache.set("k", "v")
        assert cache.get("k") == "v"
