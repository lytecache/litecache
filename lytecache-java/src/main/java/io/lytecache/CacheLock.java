 ...
 * {@link LyteCache#lock(String, Duration)}: if the holder crashes without releasing, the lock
...
public class CacheLock implements AutoCloseable {
    ...
    private final LyteCache cache;
    private final String lockKey;
    private final Duration ttl;
    private final String token = UUID.randomUUID().toString();
    private volatile boolean acquired = false;

    CacheLock(LyteCache cache, String lockName, Duration timeout) {
        this.cache = cache;
