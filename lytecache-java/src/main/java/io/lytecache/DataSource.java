...
class DataSource {
    ...
    private void ensureDirectoryExists() {
        try {
            ...
            throw new LyteCacheException("Failed to create database directory: " + e.getMessage(), e);
        }
    }
    ...
    private void initializeWriteConnection() {
        synchronized (writeLock) {
            try {
                ...
                throw new LyteCacheException("Failed to initialize write connection: " + e.getMessage(), e);
            }
        }
    }
    ...
    private Connection getReadConnection() {
        ...
        return readConnections.computeIfAbsent(threadId, k -> {
            try {
                ...
                throw new LyteCacheException("Failed to create read connection: " + e.getMessage(), e);
            }
        });
    }
