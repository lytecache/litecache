...
from litecache._schema import TYPE_BYTES, TYPE_FLOAT, TYPE_INT, TYPE_JSON, TYPE_STR
...
def test_auto_raises_on_non_json_serializable(db_path):
    cache = LiteCache(db_path, sweep_interval=None, serializer="auto")
    try:
        with pytest.raises(SerializationError):
...
def test_pickle_serializer_succeeds_on_same_object(db_path):
    cache = LiteCache(db_path, sweep_interval=None, serializer="pickle")
    try:
        cache.set("k", NotJsonable())
...
def test_auto_with_allow_pickle_false_refuses_to_read_pickled_value(db_path):
    writer = LiteCache(db_path, sweep_interval=None, serializer="pickle")
...
def test_json_mode_refuses_to_read_pickled_value(db_path):
    ...
    reader = LiteCache(db_path, sweep_interval=None, serializer="json", strict=True)
    try:
        with pytest.raises(SerializationError):
            reader.get("k")
    finally:
...
def test_json_mode_rejects_allow_pickle_at_construction(tmp_path):
    with pytest.raises(ValueError):
        LiteCache(tmp_path / "x.db", serializer="json", allow_pickle=True, sweep_interval=None)


def test_invalid_serializer_raises(tmp_path):
    with pytest.raises(ValueError):
        LiteCache(tmp_path / "x.db", serializer="bogus", sweep_interval=None)
...
def test_foreign_type_code_raises_serialization_error(db_path):
    ...
    reader = LiteCache(db_path, sweep_interval=None, strict=True)
    try:
        with pytest.raises(SerializationError):
            reader.get("java_key")
    finally:
...
def test_conformance_fixture_codes_0_to_4(db_path):
    ...
    reader = LiteCache(db_path, sweep_interval=None)
    try:
